...
function SupportScreenIframe() {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event) => {
      if (event.data.type === 'RESIZE') {
        setDimensions({
          width: event.data.width || '100%',
          height: event.data.height || '100%'
        });
      }
    };
  ...
  return (
    <div 
      className="w-full h-full bg-black flex flex-col rounded-2xl overflow-hidden"
      style={{ 
...
export default SupportScreenIframe;
